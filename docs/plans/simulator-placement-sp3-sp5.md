...
- **Settle `rolloutOccupancy` before `expansionWeight` moves off 0.** `src/engine/analyze.ts` hands the rollout's distance-closed `blocked` set to the expansion walk, so a vertex barred only by the scoring player's *own* first settlement reads as a rival dead end and stops the walk. The first pick, scored from `occupancyFromBoard`, does not have this; the Rust scorer reads a real owner array and has neither. The parity fixture only exercises the `occupancyFromBoard` path, so nothing catches the divergence. Inert at the shipped weight of 0, which is why it is deferred rather than fixed.
