...
- `src/ui/AnalysisPanel.tsx`: `displayedFactors`, `vertexDescription`, the `MenuSelect` claim picker (`setMe`), the coarse-pointer card with `Place settlement`/`Clear`, `emptyMessage` by `analysis.status` (`no-me`, `no-availability`, `no-production`, `complete`, `me-done`).
- `src/ui/analysisMarks.ts`: `recommendationMarks`, which SP3 moved out of `AnalysisPanel.tsx` so it could be unit-tested without rendering the panel.
