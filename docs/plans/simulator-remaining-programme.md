...
- [x] Policy params-file mechanism per Context (full-params files, load errors on missing or unknown keys, domain guards at load); default-params file with drift-pin test; roster round-trip test untouched — `--arm-policy label=<base>@<params.json>` (and `@` anywhere a policy name is accepted) registers `PolicyKind::Custom` through the new `policy/params_file.rs`; the key contract is checked structurally against the live structs' own serialization at every nesting level (drift-proof), gate blocks are null-or-full, `legacyValuation`/`specialBuild` are pinned; the domain guards are the engine's own `assert_params` conditions, converted to shared per-block `validate` functions; `simulator/placement/policy-default-params.json` drift-pinned in both directions; the base kind contributes only trader-family dispatch (noports and non-heuristic bases rejected); a composite-defaults params file measured bit-identical to the named composite (0 discordant of 32 paired games, trading on) and a perturbed file separates, both in the `arm_policy` e2e suite
- [x] Headroom assertion helper; committed bounds file for every swept parameter (`genericPortFactor >= 0` bound included); every screen invocation asserts headroom first — `heuristic_v1::building_band_headroom` bounds the worst building score in closed form (documented over-counts; boundary exact at `productionWeight = 100` with all else zeroed) against `NON_WINNING_CONTESTED_FLOOR`, itself pinned to the live contested expression; the loader runs guards + headroom on every params-file load, so no screen can run an inadmissible vector; `simulator/placement/sweep-bounds.json` declares ranges for every placement weight (machinery params pinned via degenerate ranges), every policy param including all four gate blocks and the J/knight axes, and the `TradeConfig` flags, with a test walking it for two-way completeness and endpoint admissibility; `EngineWeights::validate` enforces `genericPortFactor >= 0` at weights-file load
- [x] Delete SIM-GAP-30 once the bound and check ship — entry deleted; `contracts.md` gains the "Policy params files" contract and the mechanical headroom guarantee; `programme.md`'s H bullet records H0 discharging the gap; corpus recapture byte-identical (0/4000 moved), both cargo profiles + alloc green
