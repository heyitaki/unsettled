...
import { newGame, reconcileStats, type Game, type PlayerStats } from '../../model/game'
...
import { registerBoard, type Registration } from '../registration'
import { analyzeToken } from '../tokens'

export interface ParseIssue {
  stage: 'registration' | 'sharpness' | 'tiles' | 'tokens' | 'ports' | 'roster' | 'pieces' | 'stats' | 'ocr'
...
export type ParseBoardImageResult =
  | { ok: true; game: Game; issues: ParseIssue[] }
...
export abstract class ScreenshotSource<Roster extends SourceRoster = SourceRoster> {
  abstract readonly id: string

  abstract detect(image: RgbaImage): SourceDetection | null

  abstract palette(image: RgbaImage): ParserPalette | null

  abstract readRoster(image: RgbaImage, palette: ParserPalette, registration: Registration): Roster
  ...
  abstract nameRects(roster: Roster): { playerId: string; rect: Rect }[]
  ...
  parse(image: RgbaImage): SourceParse {
    ...
    try {
      ...
      const palette = this.palette(image)
      ...
      const roster = this.readRoster(image, palette, registration)
      ...
      const statDetection = this.readStats(image, palette, registration, roster)
      const game: Game = {
        ...newGame(board),
      ...
      return {
        ...
        nameRects: this.nameRects(roster),
      }
    } catch {
