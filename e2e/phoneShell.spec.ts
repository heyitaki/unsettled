...
test.describe('players screen', () => {
  test('the dots open Players, where a row claims, a name renames, and a grip drag reseats the draft', async ({ page }) => {
    ...
    await expect(rows.locator('.phone-row-name')).toHaveText(['Red', 'Blue', 'Orange', 'White'])
    await expect(screen.locator('.phone-prow.me')).toHaveCount(0)
    await expect(screen.locator('.phone-dslot')).toHaveCount(8)
    await expect(screen.locator('.phone-dslot.now')).toHaveCount(1)
    await expect(screen.locator('.phone-group-label').nth(1)).toContainText('pick 1 of 8')
    ...
    await expect(rows.nth(1).locator('.phone-you')).toBeVisible()
    await expect(rows.nth(0).locator('.phone-you')).toBeHidden()
    ...
    await field.press('Enter')
    await expect(field).toHaveCount(0)
    await expect(rows.nth(1).locator('.phone-row-name')).toHaveText('Bea')
    ...
    const grip = (await rows.nth(3).locator('.phone-grip').boundingBox())!
    ...
    await expect(screen.locator('.phone-trash')).toBeVisible()
    ...
    await page.mouse.up()
    await expect(rows.locator('.phone-row-name')).toHaveText(['White', 'Red', 'Bea', 'Orange'])
    ...
    await expect(rows.last().locator('.phone-row-name')).toHaveText('Player 5')
