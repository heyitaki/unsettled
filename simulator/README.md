...
- **G — threat-aware decisions, measured one at a time.** Robber placement is built. Monopoly and dev-card timing, trade selection and acceptance, and goal switching and denial remain. Counterparty-ranking unification is deferred to G3, where all trading consumers can move from `vp_estimate` to the same danger function together. The robber arm holds knight play/hold fixed by scoring the knight action from the self-regarding robber choice while using the threat-selected placement; rejoining timing and placement belongs to the knight-timing phase. `ThreatParams` weights are unswept Phase-H placeholders.
