...
`placement/arms/` holds the committed measurement arms. Weights files (loaded via `app_formula:`): `spread_*` scale the `resourceValue` spread, `gpf_*` vary `genericPortFactor`, `flat_swamp` renormalizes an external formula's resource ratios to the same mean, `h1_*` are the Phase-H placement-weight screen perturbations, `h3_hand_zero` is the `handValue` A/B arm (the Phase-I weights snapshot with the drop applied, byte-identical to `sp6-pre-adoption-weights.json`), `sp2a_devcard` / `sp2b_portdeficit` are the SP2 term arms and `sp3_*` the SP3 ones, each with its own new weight raised from its then-shipped 0, `sp4_*` are the SP4 slot profiles with only the four-seat `slotScales` diversity leaves moved off 1.0, `sp5_*` are the SP5 denial arms with `setupDenialWeight` alone raised, and `sp6_*` are the SP6 re-screen arms across the expansion, decay, concentration and diversity axes; the SP5 and SP6 arms are loaded on the hero path of an `app_formula_draft:` spec, which for SP5 is the only kind that reads its key and for SP6 is the reference field. Policy params files (loaded via `@`): `h2_*` and `h2x_*` are the policy-block screen arms, `h4_*` the combine and coordinate-pass vectors with `h4_final.json` the eval-confirmed candidate, `sp1e_*` the SP1e knight-and-threat sweep arms (two per axis, at its declared `sweep-bounds.json` endpoints), and `sp1r_*` the M-51 re-screen arms (one per axis M-46 adopted, put back to its pre-adoption value).

Which baseline each `h*` and `sp*` family is bracketed around, and the test pinning each, are in [`contracts.md`](../.claude/specs/simulator/contracts.md) under "Weights files". An arm re-run against a field it was not measured against is not the measurement its name records.
