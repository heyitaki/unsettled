...
`placement/arms/` holds the committed measurement arms. Weights files (loaded via `app_formula:`): `spread_*` scale the `resourceValue` spread, `gpf_*` vary `genericPortFactor`, `flat_swamp` renormalizes an external formula's resource ratios to the same mean, `h1_*` are the Phase-H placement-weight screen perturbations, `sp2a_devcard` / `sp2b_portdeficit` are the SP2 term arms and `sp3_*` the SP3 ones, each with its own new weight raised from its then-shipped 0, `sp4_*` are the SP4 slot profiles with only the four-seat `slotScales` diversity leaves moved off 1.0, `sp5_*` are the SP5 denial arms with `setupDenialWeight` alone raised, and `sp6_*` are the SP6 re-screen arms across the expansion, decay, concentration and diversity axes; the SP5 and SP6 arms are loaded on the hero path of an `app_formula_draft:` spec, which for SP5 is the only kind that reads its key and for SP6 is the reference field. Every one of these five families is a perturbation of `sp6-pre-adoption-weights.json` rather than of the live defaults, because the SP6 adoption moved `expansionWeight` after they were measured. Policy params files (loaded via `@`): `h2_*` and `h2x_*` are the policy-block screen arms, `h3_hand_zero` the `handValue` A/B arm (byte-identical to the live defaults since the M-46 handValue drop), `h4_*` the combine and coordinate-pass vectors with `h4_final.json` the eval-confirmed candidate, `sp1e_*` the SP1e knight-and-threat sweep arms (two per axis, at its declared `sweep-bounds.json` endpoints), and `sp1r_*` the M-51 re-screen arms (one per axis M-46 adopted, put back to its pre-adoption value).

The `h*` families and the `sp*` families are bracketed around different baselines: the Phase-H arms around the frozen pre-adoption defaults they were measured against, the SP arms around the live defaults. [`contracts.md`](../.claude/specs/simulator/contracts.md) under "Weights files" says which anchor applies where and names the test pinning each.
