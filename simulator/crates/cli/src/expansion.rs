...
/// The two SP0-D2 quantities for one seat's completed pair.
...
/// The share of the pair's pips sitting on its single highest-pip hex, over the distinct
/// producing hexes either settlement touches. A hex both settlements touch is counted once: the
/// robber takes it once too.
pub fn blockability_share(
    board: &SimBoard,
    topology: &Topology,
    vertex_owner: &[u8],
    seat: u8,
...
pub fn game_pairs(
    ...
    for pick in picks {
        ...
        out.push(PairReading {
            ...
            blockability: blockability_share(board, topology, vertex_owner, pick.seat),
        });
    }
}
