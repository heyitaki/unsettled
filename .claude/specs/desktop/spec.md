...
- The name is the row's only flexing cell, so every fixed cell added to its right comes straight out of it, and it must never be squeezed to nothing: the right rail's floor and ceiling both carry the `YOU` column's width, the tally narrows in the band just above the two-column arm, and the name clips inside its cell rather than keeping the phone's 30px floor and overflowing onto the chip.
