...
One `Workspace` tree for every viewport wider than 760px (mobile spec B8 holds; the phone shell is untouched). It has no tab bar and no pane gating at any width: the old `MobileNav` arm and the landscape side rail were deleted on 2026-09-03. Left rail: Library, then Board tools. Centre: the board in its paper stage with the layout caption under it. Right rail: Players (roster, then the draft ribbon), then Best picks. No tab strip and no footer. The site header is the brand mark and `Unsettled`, with no subtitle. Above 1120px the three columns sit side by side and the board is sticky; at 1120px and below the right rail wraps under the board as two equal columns and the board scrolls with the page, since Chromium bounds a sticky grid item by the whole grid and a pinned board would ride over the rail.
...
Replaces the tab strip, the `Import & export` panel and the `Saved maps` panel with one panel at the top of the left rail, the desktop's version of the phone's Maps screen (S7). It is the only place a board's name shows and the only place to switch, rename, open, delete or create one.

- **Heading**: eyebrow `Library`, h2 `Maps`, and at the right a dots button (`DotsGlyph`, `aria-label="Import and export files"`) opening a `ContextMenu` with `Import JSON`, `Export JSON`, `Back up library` and `Restore library backup`, through `useJsonFiles`. Backup includes current unsaved games, leaving out an unlinked board still blank. Restore merges without overwriting, skips content already restored under its name or a `name (n)` copy, and refuses to evict boards at the library cap.
- **`Import screenshot`**: the filled accent hero button, full width, first in the body; opens `ImportDialog`.
- The screenshot dialog keeps keyboard focus inside, closes on Escape and restores the opener's focus. The parsed board appears before optional name OCR finishes.
- **`BOARDS (n)`** group label with the four-key sort `MenuSelect` at its right (`SortGlyph` caret, `SORT_OPTIONS`, default `modifiedAt`), then the library warning when it applies, then the one list. Every edited board saves itself into a map, so open and saved are one thing and the list shows it once: first any open tab with no row among the maps (never saved, or linked to a map the listing cannot show: deleted in another window before the store caught up, or an unreadable library), then every saved map in sort order. The row of the board on screen is `current`, whichever kind it is; nothing else marks a map as loaded. Each row: hex (`boardColor(name)`), name, meta, trash (`TrashGlyph`, `aria-label="Delete <name>"`). A saved map's meta is the relative timestamp for the current sort key (`3 m ago`, no `LAST MODIFIED` prefix; `corrupt` for an invalid map); an unsaved board has none. The row switches to the board (`aria-label="Switch to <name>"`): `tab-select` for an unsaved board, `openMap` for a map, which selects the tab linked to it when one exists and loads the map into a new tab otherwise, silently either way unless the load raised board warnings. An invalid map's row is disabled. The name renames in place: `useRenameTab` for an unsaved board, `renameSavedMap` for a map (the library refresh retitles the linked tab). The trash on an unsaved board is `tab-close { discard: true }`; on a map it is `deleteSavedMap`, which deletes the map and, once that write has landed, closes the tab linked to it the same way. Discard is what keeps the autosave's rescue write for a closing tab from putting the board straight back as a new map, whether it was an unsaved board edited inside the debounce or a map just deleted. The library holds at most `MAX_MAPS` (50) maps: a new map beyond that evicts the least recently touched (newest of created, modified and opened), never the one just saved nor one this document still owes a write to, with a toast saying how many went, and the autosave closes the tab linked to an evicted map with discard so it cannot list as unsaved and re-save itself past the cap. Below the list a dashed `New board` row (`PlusGlyph`, `tab-add`), outside the scroller so it stays reachable. The list keeps its bounded scroller with the fade masks on desktop, bounded by its own max-height so a long library never runs the rail past the tools; the phone arm unbounds `.board-list` for the phone's overlay, where the one scroller is the page, never a list inside it.
...
- Landscape phones get this panel in the left rail like any other workspace viewport, which gives them the document switcher they have lacked.
...
- **Layout caption**: `4 player layout ⌄` with `ChevronGlyph`, centred under the board on both trees (the stage no longer clips, so the menu can open below it); the `19/19 terrain` and `4 pieces` counts leave the caption. `BoardCanvas` no longer consults `usePhone`. The zoom chip stays.
...
Placement analysis runs in a shared worker. A changed board shows `Analyzing placements…` until its result arrives, with no old cards or predicted marks. A failed worker offers `Retry analysis`. Board editing and navigation remain available.
...
Below the draft ribbon, `Award holders` opens Longest road and Largest army selectors when anyone qualifies. Only tied leaders can be chosen. Imported ties start as `Unknown`, while a recorded incumbent keeps a tie. Corrections share game undo, redo and persistence.
...
| DB4 | In both lists (boards, roster) the rename hit area is the name, not the row, and the field grows with the text (mobile B6). |
| DB5 | The board list reaches `tab-select`, `tab-close`, `tab-add` and the `useRenameTab` rule. `workspaceSync.ts` and the semantics of `e2e/workspaceSync.spec.ts` are unchanged: that spec changes only where a locator named the tab strip or the old open-boards list, never its pacing. |
...
| The board row, and the one list around it | `ui/ListRow.tsx` inside `ui/LibraryLists.tsx`, rendered by `ui/MapsPanel.tsx` and `phone/MapsScreen.tsx` |
...
- **No board title over the board.** The board list is the only place the active board's name shows and the only place to rename it. The site header keeps only the brand.
...
- **Landscape phones are plain workspace viewports.** They scroll the page like a short desktop window; nothing is gated behind a pane.
