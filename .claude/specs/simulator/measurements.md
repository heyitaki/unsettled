...
**It did not read `worse`, so there is no flag for the user.** The `## SP6 decisions, 2026-09-02` note in `.claude/specs/simulator/placement-programme.md` fixed one record-only extension reading with a `worse` result flagged; the flag condition was the whole selected interval below `-0.005` and the reading is `+0.717pp` with the whole interval above zero. Nothing here changes the candidate vector, and M-66 on `gate2` runs exactly as Task 15 of `docs/plans/simulator-placement-sp6.md` preregisters it.
