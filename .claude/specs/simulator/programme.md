...
- **H — re-tune weights** across the resulting grid, including the structural formula terms that were the old Phase F. This previously said to fix the scoring defects in [gaps.md](gaps.md) first; SIM-BATCH1 discharged the four named valuation defects. H0 (the params-file loader, `simulator/placement/sweep-bounds.json`, and the load-time building-band headroom check) discharged `SIM-GAP-30`: every swept vector is bounds-declared and headroom-checked before it can run. Sweep with every G gate on, for the reason M-21 gives. H3 settled the open `handValue` question below (M-43: drop recommended, ~0.1pp residual value recorded). **H1 (placement weights, M-38/M-39/M-40) and H2 (policy blocks, M-41/M-42) are screened.** H1 produced no winner at the threshold; its strongest prior is `diversityWeight` around 4.8, real but sub-threshold at 4x power. H2 produced nine `better` winners for the H4 combine, all on one surface: eight `TradeParams` axes (`etwWeight` 4.0, `tempoWeight` 0.8, `benefitWeight` 2.0, `marginScale` 24.0, `offerBase` 700, `tempoHalf` 1.0, `dangerFloor` 4.0, `dangerWeight` 2.0 — the first four still rising at their committed bounds, which per the prereg are not widened to chase them) plus `devBuyScale` 0.25, whose `better`-at-lower-scale verdict confirmed and closed `SIM-GAP-24`. Everything else measured `equivalent` or `inconclusive` inside the threshold, and much of the denial block is structurally dead at the screen's symmetric corner (zero discordant games between identical composites). **`TradeConfig` is unscreenable by this instrument, structurally (D-class, M-41):** the config is engine-global, so no perturbation can leave the field unchanged, and the hero-only estimand does not exist; its defaults stay declared placeholders, and a per-seat refusal-threshold seam is the recorded follow-up if per-seat tuning is ever wanted. The nine winners share the trade-selection machinery against a fixed field, so H4's coordinate pass and the single eval confirmation carry the interaction and field-overfit risk.
