...
Each of the four built G consumers was measured alone against a field of today's policies, and every one came back `inconclusive` with a positive point estimate; the estimates decline monotonically across all four in merge order. The entries are M-01, M-02, M-03 and M-19 in [measurements.md](measurements.md).

Some of that is the expected shape: this programme's premise is that results measured against a self-regarding field will not survive the field becoming threat-aware, which is exactly why tuning was moved to the end. But the reading G4 was supposed to settle came back the other way. G4 was called the first honest test on the grounds that denial's value does not depend on the field staying naive — and it produced the *smallest* estimate of the four. That is evidence against the comfortable reading, and it should be treated as such rather than absorbed into the same explanation as the first three.

**So H must not begin by assuming the programme is paying.** Four inconclusive results with a declining trend are consistent with "each consumer is individually under-powered" and equally consistent with "this is not paying against this field", and nothing measured so far separates those. Separating them is cheap compared to a full tuning grid: measure a *composite* arm with all four gates on against the same field, which is the one configuration never yet tested and the only one where the consumers can compound. If the composite is also inconclusive, the field is the problem and H should re-baseline against a threat-aware field before spending a grid on it. Neither held-out domain has been spent on any of this, and neither should be spent settling it.
